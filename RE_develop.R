...
res_n <- estimate_end(
  Recur(time = time, id = patient.id, event = indicator) ~ 1,
  method = "naive",
  data = a
)
res_q <- estimate_end(
...
res_np0 <- estimate_end(
  Recur(time = time, id = patient.id, event = indicator) ~ 1,
  method = "NPMLE",
  data = a,
  IPSW = FALSE
)
res_np0b <- estimate_end(
  Recur(time = time, id = patient.id, event = indicator) ~ 1,
  method = "NPMLE",
  data = a,
  IPSW = TRUE
...
res_np <- estimate_end(
  Recur(time = time, id = patient.id, event = indicator) ~ Z.1 + Z.2,
  method = "NPMLE",
  data = a,
  IPSW = FALSE
)
res_npb <- estimate_end(
  Recur(time = time, id = patient.id, event = indicator) ~ Z.1 + Z.2,
  method = "NPMLE",
  data = a,
  IPSW = TRUE
)
res_np2 <- estimate_end(
  Recur(time = time, id = patient.id, event = indicator) ~ Z.1 + Z.2 + log(xi),
  method = "NPMLE",
  data = a,
  known_recur = list(
...
res_np2b <- estimate_end(
  Recur(time = time, id = patient.id, event = indicator) ~ Z.1 + Z.2 + log(xi),
  method = "NPMLE",
  data = a,
  known_recur = list(
    S0 = function(x) pexp(x, rate = 1, lower = FALSE),
    coefs = c(1, -1, 1)
  ),
  IPSW = TRUE
)
...
curve(
  pexp(x, rate = ld0, lower = FALSE),
  from = 0,
  to = max(a$time[a$indicator == 1]),
  ylim = c(0, 1),
  ylab = ""
)
...
term <- ifelse(
  all_pts %in% possible_pts,
  rbinom(n = length(possible_pts), size = 1, prob = 0.2),
  0
)

tres_q <- estimate_end(
  Recur(time = time, id = patient.id, event = indicator, terminal = term) ~ 1,
  method = "quantile",
  data = a,
  quantile = 0.95
)
tres_np <- estimate_end(
  Recur(
    time = time,
    id = patient.id,
    event = indicator,
...
predfun <- recur_predictfun(
  ce,
  newdata = subset(df1, event == 0),
  type = "survival"
)
t0 <- seq(0, 2, by = 0.1)
